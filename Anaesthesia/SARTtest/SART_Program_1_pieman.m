...
if test == 1;
    [win, rect] = Screen('Openwindow', 0, black, [0,0,1500,800]); % Open win in small box on primary screen.
else
...
try %embedded in try-catch in case something goes wrong
    ...
    experiment_type='Please listen to the story and push the button to every digit\n\nEXCEPT the number 8';
    DrawFormattedText(win, experiment_type, 'center', 'center', white); Screen('Flip',win);
    
 
    [Sound, Fs]=audioread('pieman-short.mp3'); Sound = Sound/max(abs(Sound));
...
save([subject '_pieman_SARTdata.mat'],'data');
