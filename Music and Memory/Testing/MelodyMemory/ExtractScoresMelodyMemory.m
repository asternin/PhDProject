%navigate to folder
files=dir('*.xlsx');
results=cell(length(files),13);
for i=1:length(files);
    [~,~,raw]=xlsread(files(i).name);
    results(i,1)={files(i).name};
    raw(1,:)=[]; %remove header row
    for j=1:size(raw,1);
        if cell2mat(raw(j,3)) == 1 && strcmp(raw(j,8),'SAME')==1
            results(i,j+1)={1}; %CORRECT identification of SAME
        elseif cell2mat(raw(j,3)) == 0 && strcmp(raw(j,8),'DIFFERENT')==1
...
save('MelodyMemoryResults.mat','results')